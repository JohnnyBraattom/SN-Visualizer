    ...
    if resolved_categories:
        for category in dict.fromkeys(resolved_categories):  # preserve order, remove duplicates
            ...
            if aio_cat_df.empty:
                ...
                # Tag deep-dive: AIO visibility by domain for a specific tag within this category
                st.markdown("##### Tag deep-dive: AIO visibility by domain")
                tag_input_key = f"tag_input__{category}"
                tag_apply_key = f"tag_apply__{category}"
                tag_selected_key = f"tag_selected__{category}"

                tag_value = st.text_input("Enter a tag", key=tag_input_key)
                if st.button("Apply tag", key=tag_apply_key):
                    st.session_state[tag_selected_key] = tag_value

                selected_tag = st.session_state.get(tag_selected_key, "").strip()
                if selected_tag:
                    try:
                        tag_aio_vis_df = compute_aio_visibility_for_category_tag(df_keywords, category, selected_tag)
                    except Exception as exc:
                        st.error(f"Error computing tag AIO visibility for '{selected_tag}': {exc}")
                        tag_aio_vis_df = pd.DataFrame()

                    if tag_aio_vis_df.empty:
                        st.info(f"No AIO visibility data found for tag '{selected_tag}'.")
                    else:
