...
def _aio_present_true_mask(df: pd.DataFrame) -> pd.Series:
    """
    Robust AIO Present truth mask.

    Supports Excel exports where 'AIO Present' may be:
    - TRUE/FALSE (as booleans or strings)
    - Yes/No (legacy, as strings)
    - 1/0 (as numbers or strings)
    Blank/NaN values are treated as False.
    """
    ...
    # If already boolean, keep it (treat NaN as False)
    if s.dtype == bool:
        return s.fillna(False)

    v = s.astype(str).str.strip().str.lower()
    return v.isin({"true", "yes", "y", "1", "t"}).fillna(False)
...
def compute_aio_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    series = df["AIO Present"]
    total = len(series)
    if total == 0:
        return pd.DataFrame()

    rows = []
    mask_true = _aio_present_true_mask(df)

    for label, condition in [
        ("AIO triggered", mask_true),
        ("AIO not triggered", ~mask_true),
    ]:
...
def compute_aio_visibility(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - Only considers rows where AIO Present is TRUE (case-insensitive or boolean).
    ...
    mask_aio = _aio_present_true_mask(df)
