...
def compute_aio_visibility(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - Theoretical maximum is: for each AIO-triggering keyword,
        Search Volume * (1 + 0.8 + 0.5 + 0.1 + 0.1) = Search Volume * 2.5,
      i.e. owning the first 5 AIO positions.
    """
    ...
    # Theoretical maximum: each AIO keyword could own ranks 1-5
    theoretical_max = 0.0
    for idx, row in df_aio.iterrows():
        vol_val = row.get("Search Volume", 0)
        try:
        ...
        if vol <= 0:
            continue
        theoretical_max += vol * 2.5  # 1 + 0.8 + 0.5 + 0.1 + 0.1

    if theoretical_max <= 0:
        theoretical_max = 1.0

    rows = []
    for domain, score in visibility_scores.items():
        unique_kw = len(keyword_sets.get(domain, set()))
        share_pct = (score / theoretical_max) * 100.0
        rows.append(
            {
                "Domain": domain,
