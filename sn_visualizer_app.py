...
if uploaded_file is not None:
    ...
    # Category deep-dive
    st.subheader("4. Category deep-dive (traffic potential per tag)")
    st.write(
    ...
    if resolved_categories:
        for category in dict.fromkeys(resolved_categories):  # preserve order, remove duplicates
            ...
            try:
                cat_df = compute_category_breakdown(df_keywords, category)
            except Exception as exc:
            ...
            chart_cat = (
                ...
                .encode(
                    ...
                    y=alt.Y("Traffic_Potential:Q"),
                    tooltip=["Tag", "Keyword_Count", "Traffic_Potential"],
                )
                .properties(height=400)
            )
