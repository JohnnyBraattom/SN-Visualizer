...
def compute_aio_visibility_for_category(
    ...
    """Compute AIO visibility by domain for a category column (all non-empty values).

    Filters rows where `category_col` is non-empty (trimmed), then runs `compute_aio_visibility`
    on that subset.
    """
    if df is None or df.empty:
        return pd.DataFrame()
...
def build_enriched_excel(
    ...
    aio_category_visibility_dfs: Optional[Dict[str, pd.DataFrame]] = None,
) -> bytes:
    """Build an in-memory Excel file with original plus summary sheets."""
    output = io.BytesIO()

    def _safe_unique_sheet_name(base: str, used: set) -> str:
        # Excel constraints: max 31 chars; disallow: : \ / ? * [ ]
        ...
        while candidate in used:
            suffix = f"_{i}"
            candidate = (name[: (31 - len(suffix))] + suffix) if len(name) > (31 - len(suffix)) else (name + suffix)
            i += 1
        used.add(candidate)
        return candidate
    ...
    used_sheet_names.update({
        "Keywords",
        "Summary_Competitive",
        "Summary_TrafficPotential",
        "Summary_Seasonality",
        "Summary_AIO",
        "Summary_AIO_Visibility",
        "AIO_Tag_Visibility",
    })
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if aio_tag_visibility_df is not None and not aio_tag_visibility_df.empty:
            aio_tag_visibility_df.to_excel(writer, sheet_name="AIO_Tag_Visibility", index=False)
        ...
        if aio_category_visibility_dfs:
            for category, df_cat in aio_category_visibility_dfs.items():
                base = f"AIOVis_Cat_{str(category)[:20]}"
                sheet_name = _safe_unique_sheet_name(base, used_sheet_names)
                df_cat.to_excel(writer, sheet_name=sheet_name, index=False)
