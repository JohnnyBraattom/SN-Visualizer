...
def compute_category_breakdown(df: pd.DataFrame, category_col: str) -> pd.DataFrame:
    """
    For a given category column, compute traffic potential breakdown per tag.
    Traffic Potential = Search Volume * 0.3.
    Returns columns: Tag, Keyword_Count, Traffic_Potential.
    """
    if "Search Volume" not in df.columns:
        raise ValueError("Column 'Search Volume' not found in Keywords sheet.")
    ...
    mask = series.notna() & (series.astype(str).str.strip() != "")
    if not mask.any():
        return pd.DataFrame(columns=["Tag", "Keyword_Count", "Traffic_Potential"])
