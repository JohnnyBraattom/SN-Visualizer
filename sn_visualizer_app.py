...
if uploaded_file is not None:
    ...
    aio_vis_df = compute_aio_visibility(df_keywords, traffic_domain_map)
    if aio_vis_df.empty:
        ...
        aio_segment_df = compute_aio_segments(df_keywords, segment_cols)
        if aio_segment_df.empty:
            st.info("No AIO performance data per segment (no AIO-triggering keywords or empty segments).")
        else:
            bars_seg = (
                alt.Chart(aio_segment_df)
                .mark_bar()
                .encode(
                    x=alt.X("Segment:N"),
                    y=alt.Y("AIO_Keyword_Count:Q", axis=alt.Axis(title="AIO-triggering keywords")),
                    tooltip=["Segment", "AIO_Keyword_Count", "AIO_Theoretical_Max"],
                )
            )
            ...
            line_seg = (
                ...
                .encode(
                    x=alt.X("Segment:N"),
                    y=alt.Y(
                        "AIO_Theoretical_Max:Q",
                        axis=alt.Axis(title="Theoretical max AIO visibility (SV)", orient="right"),
                    ),
                    tooltip=["Segment", "AIO_Theoretical_Max"],
                )
            )

            chart_aio_seg = (
                alt.layer(bars_seg, line_seg)
                .resolve_scale(y="independent")
                .properties(height=400)
            )

            st.altair_chart(chart_aio_seg, use_container_width=True)
            with st.expander("Show AIO performance by segment data"):
                st.dataframe(aio_segment_df, use_container_width=True)
    ...
    if resolved_categories:
        for category in dict.fromkeys(resolved_categories):  # preserve order, remove duplicates
            ...
            if aio_cat_df.empty:
                st.info(f"No AIO-triggering keywords found for category '{category}'.")
            else:
                bars_aio_cat = (
                    alt.Chart(aio_cat_df)
                    .mark_bar()
                    .encode(
                        x=alt.X("Tag:N", sort="-y"),
                        y=alt.Y("AIO_Keyword_Count:Q", axis=alt.Axis(title="AIO-triggering keywords")),
                        tooltip=["Tag", "AIO_Keyword_Count", "AIO_Theoretical_Max"],
                    )
                )
                ...
                line_aio_cat = (
                    ...
                    .encode(
                        ...
                        tooltip=["Tag", "AIO_Theoretical_Max"],
                    )
                )

                chart_aio_cat = (
                    alt.layer(bars_aio_cat, line_aio_cat)
                    .resolve_scale(y="independent")
                    .properties(height=400)
                )
    ...
    try:
        excel_bytes = build_enriched_excel(
            ...
            aio_segments_df=aio_segment_df,
