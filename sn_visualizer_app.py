...
from sn_visualizer_backend import (
    ...
    compute_aio_visibility_for_category_tag,
    compute_aio_visibility_for_category,
...
if uploaded_file is not None:
    ...
    aio_category_visibility_dfs = {}
    aio_tag_visibility_rows = []
    if resolved_categories:
        for category in dict.fromkeys(resolved_categories):  # preserve order, remove duplicates
            ...
            if aio_cat_df.empty:
                ...
                aio_metric_cols = [c for c in aio_cat_df.columns if c != "Tag"]
                aio_cat_long = aio_cat_df.melt(
                    id_vars=["Tag"],
                    value_vars=aio_metric_cols,
                    var_name="Metric",
                    value_name="Value",
                )
                ...
                # AIO visibility by domain for this category (all non-empty values in the category column)
                st.markdown("##### AIO visibility by domain for this category")
                try:
                    aio_cat_vis_df = compute_aio_visibility_for_category(df_keywords, category)
                except Exception as exc:
                    st.error(f"Error computing category AIO visibility for '{category}': {exc}")
                    aio_cat_vis_df = pd.DataFrame()

                if aio_cat_vis_df.empty:
                    st.info(f"No AIO visibility data could be computed for category '{category}'.")
                else:
                    # Add to export (one sheet per category)
                    aio_category_visibility_dfs[category] = aio_cat_vis_df

                    max_score_cat = aio_cat_vis_df["AIO Visibility Score (%)"].max()
                    if pd.isna(max_score_cat) or max_score_cat <= 0:
                        max_score_cat = 1.0
                    if max_score_cat <= 20:
                        y_max_pct_cat = 20
                    elif max_score_cat <= 50:
                        y_max_pct_cat = 50
                    else:
                        y_max_pct_cat = 100

                    cat_bars = (
                        alt.Chart(aio_cat_vis_df)
                        .mark_bar()
                        .encode(
                            x=alt.X("Domain:N", sort="-y"),
                            y=alt.Y("Unique AIO Keywords:Q", axis=alt.Axis(title="Unique AIO Keywords")),
                    ...
                    cat_line = (
                        ...
                        .encode(
                            ...
                            y=alt.Y(
                                "AIO Visibility Score (%):Q",
                                scale=alt.Scale(domain=[0, y_max_pct_cat]),
                                axis=alt.Axis(title="AIO Visibility Score (%)", orient="right"),
                            ),
                            tooltip=["Domain", "AIO Visibility Score (%)"],
                            color=alt.value("red"),
                    ...
                    display_cols_cat = [c for c in aio_cat_vis_df.columns if c != "AIO Visibility (Weighted Volume)"]
                    with st.expander(f"Show data for {category} (AIO visibility)"):
                        st.dataframe(aio_cat_vis_df[display_cols_cat], use_container_width=True)

                # Tag deep-dive: AIO visibility by domain for tags within this category
                st.markdown("##### Tag deep-dive: AIO visibility by domain")
                tags_key = f"tag_list__{category}"
                tag_input_key = f"tag_input__{category}"
                if tags_key not in st.session_state:
                    st.session_state[tags_key] = []
                
                tag_value = st.text_input("Enter a tag", key=tag_input_key).strip()
                c1, c2 = st.columns([1, 1])
                with c1:
                    if st.button("Add tag", key=f"tag_add__{category}") and tag_value:
                        if tag_value not in st.session_state[tags_key]:
                            st.session_state[tags_key].append(tag_value)
                with c2:
                    if st.button("Clear tags", key=f"tag_clear__{category}"):
                        st.session_state[tags_key] = []
                
                tags_for_category = list(st.session_state.get(tags_key, []))
                if tags_for_category:
                    st.caption("Active tags (kept until you clear them):")
                    for t in tags_for_category:
                        r1, r2 = st.columns([6, 1])
                        with r1:
                            st.write(t)
                        with r2:
                            if st.button("Remove", key=f"tag_remove__{category}__{t}"):
                                st.session_state[tags_key] = [x for x in st.session_state[tags_key] if x != t]
                                st.rerun()
                
                for t in list(st.session_state.get(tags_key, [])):
                    st.markdown(f"###### Tag: {t}")
                    show_opts = ["All domains"]
                    if client_domain:
                        show_opts.append(f"Client only ({client_domain})")
                        show_opts.append("Competitors only")
                    show_choice = st.selectbox("Show data for", options=show_opts, key=f"tag_show__{category}__{t}")
                    try:
                        tag_aio_vis_df = compute_aio_visibility_for_category_tag(df_keywords, category, t)
                    except Exception as exc:
                        st.error(f"Error computing tag AIO visibility for '{t}': {exc}")
                        tag_aio_vis_df = pd.DataFrame()
                
                    if not tag_aio_vis_df.empty:
                        # Apply Show data for filter
                        if client_domain and show_choice.startswith("Client only"):
                            tag_aio_vis_df = tag_aio_vis_df[tag_aio_vis_df["Domain"] == client_domain].copy()
                        elif client_domain and show_choice == "Competitors only":
                            tag_aio_vis_df = tag_aio_vis_df[tag_aio_vis_df["Domain"] != client_domain].copy()
                
                    if tag_aio_vis_df.empty:
                        st.info(f"No AIO visibility data found for tag '{t}'.")
                    else:
                        # Add to export (stacked sheet)
                        export_df = tag_aio_vis_df.copy()
                        export_df.insert(0, "Tag", t)
                        export_df.insert(0, "Category", category)
                        aio_tag_visibility_rows.append(export_df)
                
                        max_score_tag = tag_aio_vis_df["AIO Visibility Score (%)"].max()
                        if pd.isna(max_score_tag) or max_score_tag <= 0:
                            max_score_tag = 1.0
                        if max_score_tag <= 20:
                            y_max_pct_tag = 20
                        elif max_score_tag <= 50:
                            y_max_pct_tag = 50
                        else:
                            y_max_pct_tag = 100
                
                        tag_bars = (
                            alt.Chart(tag_aio_vis_df)
                            .mark_bar()
                            .encode(
                                x=alt.X("Domain:N", sort="-y"),
                                y=alt.Y("Unique AIO Keywords:Q", axis=alt.Axis(title="Unique AIO Keywords")),
                                tooltip=["Domain", "Unique AIO Keywords"],
                            )
                        )
                        ...
                        tag_line = (
                            ...
                            .encode(
                                ...
                                y=alt.Y(
                                    ...
                                    scale=alt.Scale(domain=[0, y_max_pct_tag]),
                                ),
                                tooltip=["Domain", "AIO Visibility Score (%)"],
                                color=alt.value("red"),
                            )
                        ...
                        tag_chart = (
                            alt.layer(tag_bars, tag_line)
                            .resolve_scale(y="independent")
                            .properties(height=400)
                        )
                
                        st.altair_chart(tag_chart, use_container_width=True)
                        with st.expander(f"Show data for tag '{t}'"):
                            st.dataframe(tag_aio_vis_df, use_container_width=True)
    ...
    # Collate tag-level AIO visibility results for export (if any)
    aio_tag_visibility_df = pd.DataFrame()
    if aio_tag_visibility_rows:
        try:
            aio_tag_visibility_df = pd.concat(aio_tag_visibility_rows, ignore_index=True)
        except Exception:
            aio_tag_visibility_df = pd.DataFrame()
    ...
    try:
        excel_bytes = build_enriched_excel(
            ...
            aio_tag_visibility_df=aio_tag_visibility_df,
            category_breakdowns=category_breakdowns,
            aio_category_breakdowns=aio_category_breakdowns,
            aio_category_visibility_dfs=aio_category_visibility_dfs,
        )
