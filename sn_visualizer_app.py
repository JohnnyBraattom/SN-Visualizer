...
if uploaded_file is not None:
    ...
    if resolved_categories:
        for category in dict.fromkeys(resolved_categories):  # preserve order, remove duplicates
            ...
            # AIO visibility by domain for this category
            ...
            try:
                ...
                st.error(f"Error computing AIO visibility for {category}: {exc}")
                aio_cat_vis_df = pd.DataFrame()

            if aio_cat_vis_df is None or aio_cat_vis_df.empty:
                st.info(f"No AIO visibility data found for category {category}.")
            else:
                y_max_pct = float(aio_cat_vis_df["AIO Visibility Score (%)"].max())
                if y_max_pct <= 100:
                    y_max_pct = 100
                y_max_pct = min(1000, y_max_pct * 1.2)
                ...
                bars = (
                    ...
                    .encode(
                        ...
                        tooltip=[
                            "Domain",
                            "Unique AIO Keywords",
                            alt.Tooltip("AIO Visibility Score (%):Q", format=".2f"),
                        ],
                    )
                )
                ...
                chart_aio_cat_vis = alt.layer(bars, line).resolve_scale(y="independent").properties(height=400)
                st.altair_chart(chart_aio_cat_vis, use_container_width=True)
                with st.expander(f"Show AIO visibility data for {category}"):
                    st.dataframe(aio_cat_vis_df, use_container_width=True)

                category_aio_visibility[category] = aio_cat_vis_df
    ...
    try:
        excel_bytes = build_enriched_excel(
            ...
            category_aio_visibility=category_aio_visibility,
