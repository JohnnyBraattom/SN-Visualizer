...
def compute_category_breakdown(df: pd.DataFrame, category_col: str) -> pd.DataFrame:
    """
    For a given category column, compute search volume breakdown per tag.
    Returns columns: Tag, Keyword Count, Search_Volume.
    """
    if "Search Volume" not in df.columns:
        raise ValueError("Column 'Search Volume' not found in Keywords sheet.")
    ...
    mask = series.notna() & (series.astype(str).str.strip() != "")
    if not mask.any():
        return pd.DataFrame(columns=["Tag", "Keyword_Count", "Search_Volume"])
    ...
    agg = agg.sort_values("Search_Volume", ascending=False).reset_index(drop=True)
    return agg
...
def build_enriched_excel(
    ...
    aio_summary_df: pd.DataFrame | None = None,
    category_breakdowns: Dict[str, pd.DataFrame] | None = None,
) -> bytes:
